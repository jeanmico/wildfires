...
crs(sensors_full) <- CRS("+proj=longlat +datum=NAD83")  # WGS84

sensors_water <- sensors_full
crs(sensors_water) <- proj4string(CRS("+proj=longlat +datum=NAD83"))
...
For each day for each watershed, interpolate (default is kriging)
```{r}
library(lubridate)
...
```
```{r}
rast_basename = '/Volumes/Padlock/wildfires/rast_water/'
...
# map the interpolated raster
rast_plt <- tm_shape(r_clip, bbox = pa_sp) +
