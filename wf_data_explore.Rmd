...
time_overlap <- function(df, varname, noexp_value) {
  ...
  df <- df %>% mutate(overlap1 = case_when(int_overlaps(tri1, tmpintvl) ~ 
                                                     as.duration(intersect(tri1, tmpintvl)),
                                                   TRUE ~ 0),
                              overlap2 = case_when(int_overlaps(tri2, tmpintvl) ~ 
                                                     as.duration(intersect(tri2, tmpintvl)),
                                                   TRUE ~ 0),
                              overlap3 = case_when(int_overlaps(tri3, tmpintvl) ~ 
                                                     as.duration(intersect(tri3, tmpintvl)),
                                                   TRUE ~ 0) )
  ...
  if (!noexp_value) {
    ...
    df <- df %>% mutate(total_overlap_days = case_when(
      preg_fire_ovrlp ~ as.numeric(intersect(tmpintvl, preg_intvl), 'days'),
      TRUE ~ 0))
  }
...
cont_tbl <- function(df, tri, tert) {
  ...
  tst <- epitable(tmpdf$binary_exposed, tmpdf$preterm)
...
```{r loglinear}
loglin <- function(df, expvar, fire_only) {
  # df <- df %>% filter(train == TRUE)
  tmpmdl = glm(preterm ~ . , data = df, family = 'poisson')
  ...
  if (fire_only == TRUE) {
    ...
    write.csv(tmpdf, file = output_name(paste('loglinear', expvar, sep = '_')), row.names = FALSE)
    }
  }
...
partcov <<- c("preterm", "race", "ipi", "edu", "insurance", "pn_care", "wic", "mom_age"
              )
...
births = births %>% mutate(nulliparous = case_when(nulliparous == 1 ~ 1,
                                                   TRUE ~ 0))
...
births <- births %>% mutate(preterm = case_when(
  gest_weeks < 37 ~ 1,
  TRUE ~ 0))
births <- births %>% mutate(pt_early = case_when(gest_weeks < 28 ~ 1, TRUE ~ 0),
                            pt_mid = case_when(gest_weeks < 36 & gest_weeks > 28 ~ 1, TRUE ~ 0),
                            pt_late = case_when(gest_weeks < 38 & gest_weeks > 35 ~ 1, TRUE ~0))

births <- births %>% mutate(outcome = preterm)
```
...
```{r trimester overlap}
births_tmp = births
births_tmp = trimester_overlap(births)
...
```{r check work}
library(tableone)
mytab <- CreateTableOne(vars = partcov, data = births, factorVars = c('preexist_HTN', 'preexist_diabetes', 'mental_disorder', 'nulliparous'))
...
ptb_rate = nrow(births %>% filter(preterm == 1))/nrow(births)
ptb_rate
...
#timeplt
#ggsave(timeplt, file = image_name('timeplot'), width = 12, height = 7)
```
...
```{r exposure violins}
meanexp_box = ggplot(births, aes(x = binary_exposed, y = pm25mean)) +
                geom_boxplot() + 
  theme_bw()

meanexp_box

ggsave(meanexp_box, file = image_name('meanexp_fullpreg_boxplot'))
```
...
tst <- epitable(births_before$binary_exposed, births_before$preterm)

epitab(tst, method = 'riskratio')
...
tst_3tri <- epitable(births$exposure_tri, births$preterm) 
...
tst_htri <- epitable(births$halftri_exp, births$preterm) 
...
htri_risk = ggplot(epi_htri, aes(x = htri, y = tab.riskratio, ymin = tab.lower, ymax = tab.upper)) + 
  ...
  ylab("Risk Ratio of Preterm Birth") + 
  xlab("Half Trimester") + 
  theme_bw()
...
```{r pm25exposure logit}

simplecont_exp <- glm(preterm ~  pm25mean, data = births_trn, family = 'poisson')

summary(simplecont_exp)  # pm25 mean is significant here

```


Test a very simple log linear model: outcome is preterm birth (binary), input is covariates and trimester of exposure.

```{r models: all, trimester of exposure}
expvars = c('binary_exposed', 'exposure_tri', 'pm25count_50', 'pm25count_100', 'pm25mean')

for (i in 1:length(expvars)) {
  mdf = births_trn %>% dplyr::select(c(all_of(partcov)), expvars[[i]])
  loglin(mdf, expvars[[i]], FALSE)
...
fire_births_map = exp_births %>% dplyr::select(VS_unique, x, y, pm25mean, pm25mean_exp, pm25peak, preterm)
...
exp_3tri <- epitable(exp_births$exposure_tri, exp_births$preterm, rev = "rows") 
...
firetert_tbl <- epitable(exp_births$ftert_desc, exp_births$preterm)
...
ftert_log = glm(preterm ~ race + ipi + edu + insurance + pn_care + bmi + wic + preexist_HTN + preexist_diabetes +  firetert, data = exp_births, family = 'poisson')
