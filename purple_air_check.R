# double check purple air
library(dplyr)
library(lubridate)
library(tidyr)
library(ggplot2)

processed_pa_all = read.csv('~/wildfires/2018_purpleair.csv')

processed_pa <- processed_pa_all %>% filter(ymd(day_date) < ymd('2018-12-01') & ymd(day_date) > ymd('2018-10-31'))

processed_pm25 <- processed_pa %>% dplyr::select(id, lat, lon, day_date, mean_pm25_1, pm25_1_corr) %>%
  pivot_longer(cols = c('mean_pm25_1', 'pm25_1_corr'), names_to = 'measure_type', values_to = 'pm25')

epa= read.csv('~/wildfires/2018_CA_PM25_EPA.csv')
epa <- epa %>% mutate(day_date = mdy(Date)) %>% filter(day_date < ymd('2018-12-01') & day_date > ymd('2018-10-31') & CBSA_CODE == 41860)
